...
@service.route('/get_service_list',methods=('GET','POST'))
...
#列出ingress
@service.route('/get_ingress_list',methods=('GET','POST'))
...
@service.route('/delete_service', methods=('GET', 'POST'))
def delete_service():
    # myclient = client.CoreV1Api()
...
@service.route('/delete_ingress', methods=('GET', 'POST'))
def delete_ingress():
    data = json.loads(request.get_data().decode('utf-8'))
    name  = handle_input(data.get('name'))
    namespace = handle_input(data.get('namespace'))
